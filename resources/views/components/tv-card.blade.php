...
<a href="{{route('movies.show',$popularShow['id'])}}" class="text-sm mt-2 hover:text-gray-300">
      ...
      <div class="flex items-center text-gray-400 text-sm">
                      ...
                      <span class="ml-1 text-sm">{{ $popularShow['vote_average'] *10 .'%' }}</span>
                      <span class="mx-2">|</span>
                      <span class="text-sm">{{ \Carbon\Carbon::parse($popularShow['first_air_date'])->format('M d, Y') }}</span>


                  </div>
                  <div class="text-gray-400 text-sm">

                  <span class="text-sm">@foreach ($popularShow['genre_ids'] as $genre )
